...
static const AVOption zstr_srt_mux_options[] = {
    { "host",         "Host or IP address", OFFSET(host),         AV_OPT_TYPE_STRING, { .str = "127.0.0.1" }, 0, 0, ENC },
    { "port",         "Port number",        OFFSET(port),         AV_OPT_TYPE_INT,    { .i64 = 9000 },        0, 65535, ENC },
    { "mode",         "Connection mode",    OFFSET(mode_str),     AV_OPT_TYPE_STRING, { .str = "caller" },   0, 0, ENC },
    { "latency",      "Latency in ms",      OFFSET(latency),      AV_OPT_TYPE_INT,    { .i64 = 120 },         0, 10000, ENC },
    { "passphrase",   "AES Passphrase",     OFFSET(passphrase),   AV_OPT_TYPE_STRING, { .str = NULL },        0, 0, ENC },
    { "pbkeylen",     "Key length in bytes",OFFSET(pbkeylen),     AV_OPT_TYPE_INT,    { .i64 = 16 },          16, 32, ENC },
    { "streamid",     "Stream ID",          OFFSET(streamid),     AV_OPT_TYPE_STRING, { .str = NULL },        0, 0, ENC },
    { "payload_size", "Payload size",       OFFSET(payload_size), AV_OPT_TYPE_INT,    { .i64 = 1316 },        0, 65536, ENC },
    { "timeout",      "Timeout in ms",      OFFSET(timeout_ms),   AV_OPT_TYPE_INT,    { .i64 = 3000 },        0, 60000, ENC },
    { NULL }
};
