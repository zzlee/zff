/*=============================================================================
    zstr_webrtc_twcc.c — Transport-Wide Congestion Control (TWCC / GCC)

    Ported from zstreamer `zst_webrtc_twcc.c`. Framework deltas:
    - zst_bus REMB events -> zstr_twcc_bitrate_cb callback
    - ZST_LOG_* -> av_log
=============================================================================*/
...
struct zstr_webrtc_twcc {
    zstr_twcc_bitrate_cb bitrate_cb;
    void         *cb_user_data;
    int           extmap_id;
    uint16_t      seq_num;
 ...
 *   ≈ 0 → normal
 */
static void gcc_delay_update(zstr_webrtc_twcc_t* twcc, float delta_delay_ms) {
    ...
    if (twcc->bw_state != prev_state) {
        av_log(NULL, AV_LOG_DEBUG, "twcc: " "BW state: %s → %s (grad=%.2f threshold=%.2f)",
...
static void gcc_delay_adapt(zstr_webrtc_twcc_t* twcc) {
...
static void gcc_loss_adapt(zstr_webrtc_twcc_t* twcc, float loss_rate) {
 ...
 */
static void twcc_parse_ccfb(zstr_webrtc_twcc_t* twcc, const uint8_t* msg, int size) {
    if (size < 20) return;
    ...
    if (prev == 0 || diff * 100 / prev > 3) {
        ...
        if (twcc->bitrate_cb)
            twcc->bitrate_cb(combined, twcc->cb_user_data);
...
void* zstr_webrtc_twcc_process_incoming(zstr_webrtc_twcc_t* twcc, const char* message, int size) {
...
void* zstr_webrtc_twcc_process_outgoing(zstr_webrtc_twcc_t* twcc, const char* message, int size) {
...
zstr_webrtc_twcc_t* zstr_webrtc_twcc_create(zstr_twcc_bitrate_cb cb,
                                            void *user_data) {
    zstr_webrtc_twcc_t* twcc = calloc(1, sizeof(zstr_webrtc_twcc_t));
    if (!twcc) return NULL;
    twcc->bitrate_cb          = cb;
    twcc->cb_user_data        = user_data;
    twcc->extmap_id           = -1;
    twcc->seq_num             = 1;
...
int zstr_webrtc_twcc_parse_offer(zstr_webrtc_twcc_t* twcc, const char* offer_sdp) {
    ...
    if (strncmp(start, extmap_str, strlen(extmap_str)) == 0) {
        ...
        twcc->extmap_id = id;
        av_log(NULL, AV_LOG_INFO, "twcc: " "TWCC extmap ID from offer: %d", id);
        return id;
    }
    return -1;
}

int zstr_webrtc_twcc_inject_answer(zstr_webrtc_twcc_t* twcc, char* answer_sdp, size_t max_len) {
...
uint64_t zstr_webrtc_twcc_bitrate(const zstr_webrtc_twcc_t *twcc) {
    return twcc ? twcc->current_bitrate_bps : 0;
}
